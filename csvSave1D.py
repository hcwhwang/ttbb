...
rootFile = 'csvNbjetsL3_1D_3rd.root'
outputJson = 'outputNbjetsL3_1D_3rd.json'
...
for i,d in enumerate(mceventweight):
  ...
  var = "jets_bDiscriminatorCSV[csvd_jetid[2]]"

  hBkgAllName = "Bkg_" + tree + "_" + d["name"]
  hBkgAll = TH1F(hBkgAllName,"",binning[0], binning[1], binning[2])

  hRDAllName = "Data_" + tree + "_" + d["name"]
  hRDAll = TH1F(hRDAllName,"",binning[0],binning[1], binning[2])
  if i==0:
    for i,file in enumerate(RDList):
      ...
      t = f.Get(treepath)
      h = TH1F(histName,"",binning[0], binning[1], binning[2])
  ...
  for i,file in enumerate(DYList):
    ...
    wentries = getWeightedEntries(fileName, nomtreepath, "filtered", PUweight)
    scale = datalumi*data["xsec"]/wentries
    hDY = TH1F(histName,"",binning[0], binning[1], binning[2])    
    for j,ch in enumerate(ChList):
      ...
      h = TH1F(histDYChName,"",binning[0], binning[1], binning[2])
  ...
  for i,ttName in enumerate(TTList):
    ...
    nttjjR = getWeightedEntries(fileName, treepath, "filtered", weightCut(mAND(visible,bkgCut),weight),scale)
    nttbbR = getWeightedEntries(fileName, treepath, "filtered", weightCut(mAND(ttbb,bkgCut),weight),scale)
