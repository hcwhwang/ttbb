...
TTList = ['TT_powheg', 'TT_powheg_FSR_Up', 'TT_powheg_FSR_Down', 'TT_powheg_ISR_Down', 'TT_powheg_ISR_Up', 'TT_powheg_UE_Up', 'TT_powheg_UE_Down']
  ...
  for ch in range(3):
    ...
    for i,ttName in enumerate(TTList):
      ...
      if d["name"]=="Ttcc_Fraction_Up":
        ...
        nttcclfR -= getWeightedEntries2D(fileName, treepath, var, weightCut(mAND(mAND(ttcc,overallCut),ChCutList[ch]),weight),0.5*scale)
      '''
      #out = {'name':d['name'], 'channel':ChList[ch], 'tot':datalumi*data["xsec"], 'nttjjF':nttjjF, 'nttbbF':nttbbF, 'nttjjV':nttjjV, 'nttbbV':nttbbV, 'nttjjR':nttjjR, 'nttbbR':nttbbR}
      #output.append(out)
      #print out 
