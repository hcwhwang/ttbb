...
datalumi = 35.82*1000
RDList = ['MuonEG_Run2016_tot', 'DoubleEG_Run2016_tot', 'DoubleMuon_Run2016_tot']
...
for sys,d in enumerate(mceventweight):
  ...
  print d["name"],sys
  #if "JES" in d["name"] : rootdir = "/xrootd/store/user/chanwook/ttbb/final/preapproval/JES/"
  if "JES" in d["name"] : rootdir = "./JES/"
  #else : rootdir = "/xrootd/store/user/chanwook/ttbb/final/preapproval/"
  else : rootdir = "./"
  #if sys!=0: continue
  #if sys < 33: continue
  ...
  for ch in range(3):
    ...
    for i,file in enumerate(DYList):
      ...
      #hDY.Add(hh)
      #nDY += hAndN[1]
    if d["name"].split("_")[0] == 'MuF' or d["name"].split("_")[0] == 'MuR'or d["name"].split("_")[0] == 'PDF':
    ...
    #print out 
    print "init r_ttbb = ", nttbbR/nttjjR
    print "init r_ttbj = ", nttbjR/nttjjR
    ...
    print "efficiency(ttbb,ttjj) : ", nttbbR/nttbbV, nttjjR/nttjjV
    print "acceptance(ttbb,ttjj) : ", nttbbV/nttbbF, nttjjV/nttjjF
    if ttName == 'TT_powheg': 
      f1 = TFile.Open(rootdir + 'TT_powheg_tot.root')
      t1 = f1.Get(treepath)
      scale = datalumi*831.76/(wentries + wentriesTTLL*(1./0.104))
      print "TTLL+ TT scale : ", scale
