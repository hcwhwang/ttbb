...
class TtbarBbbarDiLeptonSingleElectronAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit TtbarBbbarDiLeptonSingleElectronAnalyzer(const edm::ParameterSet&);
  ~TtbarBbbarDiLeptonSingleElectronAnalyzer();
...
private:
  ...
  void selectMuons(const cat::MuonCollection& muons, cat::MuonCollection& selmuons, TtbarBbbarDiLeptonSingleElectronAnalyzer::sys_e sys) const;
  void selectElecs(const cat::ElectronCollection& elecs, cat::ElectronCollection& selelecs, TtbarBbbarDiLeptonSingleElectronAnalyzer::sys_e sys) const;
  //  cat::JetCollection selectJets(const cat::JetCollection& jets, const LeptonCollection& recolep) const;
  cat::JetCollection selectJets(const cat::JetCollection& jets, const LeptonPtrs& recolep, TtbarBbbarDiLeptonSingleElectronAnalyzer::sys_e sys);
...
//
TtbarBbbarDiLeptonSingleElectronAnalyzer::TtbarBbbarDiLeptonSingleElectronAnalyzer(const edm::ParameterSet& iConfig)
{
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::book(TTree* tree){
...
TtbarBbbarDiLeptonSingleElectronAnalyzer::~TtbarBbbarDiLeptonSingleElectronAnalyzer()
{
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
////////////////////////////////////////
  for (int sys = 0; sys < nsys_e; ++sys){
    ...
    if (b_channel == CH_MUEL && b_tri_singleM == 0 && b_tri_singleE > 0 && b_tri_di == 0) b_tri = 1;
    else if (b_channel == CH_ELEL && b_tri_di == 0 && b_tri_single > 0) b_tri = 1;
    else b_tri = 0;
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::selectMuons(const cat::MuonCollection& muons, cat::MuonCollection& selmuons,sys_e sys) const
{
  for (auto& m : muons) {
    cat::Muon mu(m);
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::selectElecs(const cat::ElectronCollection& elecs, cat::ElectronCollection& selelecs, sys_e sys) const
{
  for (auto& e : elecs) {
    cat::Electron el(e);
...
cat::JetCollection TtbarBbbarDiLeptonSingleElectronAnalyzer::selectJets(const cat::JetCollection& jets, const TtbarBbbarDiLeptonSingleElectronAnalyzer::LeptonPtrs& recolep, sys_e sys)
{
...
cat::JetCollection TtbarBbbarDiLeptonSingleElectronAnalyzer::selectBJets(const JetCollection& jets, double workingpoint) const
{
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::resetBr()
{
    resetBrGEN();
    resetBrReco();
}
void TtbarBbbarDiLeptonSingleElectronAnalyzer::resetBrReco()
{
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::resetBrJets()
{
...
void TtbarBbbarDiLeptonSingleElectronAnalyzer::resetBrGEN()
{
  b_genTtbarId=0;
  b_pdfWeights.clear();
...
DEFINE_FWK_MODULE(TtbarBbbarDiLeptonSingleElectronAnalyzer);
