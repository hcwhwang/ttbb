...
def make_legend(xmin,ymin,xmax,ymax):
  ...
  return leg
name = "Nbjets2"
result = "fitresult%s_pdf.txt" % name
rootFile = "csv%s_pdf.root" % name
...
for i,d in enumerate(mceventweight):
  ...
  k      =RooRealVar(       "k","normalization factor",           1, 0.0, 1.5)
  ...
  if not freeTTB :line = "%s %f %f %f %f %f %f\n" % (d["name"], fttbb.getVal(), fttbb.getError(), k.getVal(), k.getError(), fttbjcon.getVal(), rttbb)
  else :line = "%s %f %f %f %f %f %f\n" % (d["name"], fttbb.getVal(), fttbb.getError(), k.getVal(), k.getError(), fttbj.getVal(), rttbb)
  file.write(line)
  file.close()
  #continue
  if mceventweight[i]["name"] not in ["csvweight","Free_ttbj"]: continue
  ...
  pt.Draw()
  cR10.SaveAs("fttbbNLL_%s_%s.png"%(mceventweight[i]["name"],name))
  ...
  pt.Draw()
  cR00.SaveAs("kNLL_%s_%s.png"%(mceventweight[i]["name"],name))
  ...
  pt.Draw()
  cR11.SaveAs("csv4th_%s_%s.png"%(mceventweight[i]["name"],name))
  ...
  pt.Draw()
  cR12.SaveAs("csv3rd_%s_%s.png"%(mceventweight[i]["name"],name))
  if freeTTB:
    ...
    l2.AddEntry(preM,"prefit: R2="+str(round(rttbj,4)),"p")
    l2.AddEntry(preM23,"fit(cen.): R="+str((0.0421)) ,"p")
    l2.AddEntry(preM23,"fit(cen.): R2="+str((0.1064)),"p")
  
    l2.AddEntry(preM2,"fit: R="+str(round(fttbb.getVal(),5))+" #pm "+str(round(fttbb.getError(),5))+"","p")
    l2.AddEntry(preM2,"fit: R2="+str(round(fttbj.getVal(),5))+" #pm "+str(round(fttbj.getError(),5))+"","p")
    l2.SetTextSize(0.04)
    l2.SetFillColor(0)
    l2.SetLineColor(0)
    l2.Draw()    
    cR13.SaveAs("ttbbVsTtbj_%s_%s.png"%(mceventweight[i]["name"],name))
f.Close()
