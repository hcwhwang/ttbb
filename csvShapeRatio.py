...
for i,sys in enumerate(sysList):  
  ...
  for j in range(4):
    ...
    canName = "csv_%i_%s_Ratio_Nbjets" % (j+1,sys)
    ...
    #hCenUpList[0].GetYaxis().SetTitle("Cen/Up")
    #hCenDnList[0].GetYaxis().SetTitle("Cen/Down")
    hFrame.SetMinimum(0)
    #hMax = max(h.GetMaximum()/h.Integral() for h in hList)
    hMax = max(h.GetMaximum() for h in hList)
    ...
    for l,cut in enumerate(ttbarid_cut): 
      ...
      hCenUpList[l].Divide(hList[l+4])
      hCenUpList[l].SetMaximum(1.2)      
      hCenUpList[l].SetMinimum(0.8)
      print "Test2"
    ...
    for l,cut in enumerate(ttbarid_cut): 
      ...
      hCenDnList[l].Divide(hList[l+8])
      hCenDnList[l].SetMaximum(1.2)      
      hCenDnList[l].SetMinimum(0.8)
