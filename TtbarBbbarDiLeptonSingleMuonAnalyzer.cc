...
class TtbarBbbarDiLeptonSingleMuonAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit TtbarBbbarDiLeptonSingleMuonAnalyzer(const edm::ParameterSet&);
  ~TtbarBbbarDiLeptonSingleMuonAnalyzer();
...
private:
  ...
  void selectMuons(const cat::MuonCollection& muons, cat::MuonCollection& selmuons, TtbarBbbarDiLeptonSingleMuonAnalyzer::sys_e sys) const;
  void selectElecs(const cat::ElectronCollection& elecs, cat::ElectronCollection& selelecs, TtbarBbbarDiLeptonSingleMuonAnalyzer::sys_e sys) const;
  //  cat::JetCollection selectJets(const cat::JetCollection& jets, const LeptonCollection& recolep) const;
  cat::JetCollection selectJets(const cat::JetCollection& jets, const LeptonPtrs& recolep, TtbarBbbarDiLeptonSingleMuonAnalyzer::sys_e sys);
...
//
TtbarBbbarDiLeptonSingleMuonAnalyzer::TtbarBbbarDiLeptonSingleMuonAnalyzer(const edm::ParameterSet& iConfig)
{
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::book(TTree* tree){
...
TtbarBbbarDiLeptonSingleMuonAnalyzer::~TtbarBbbarDiLeptonSingleMuonAnalyzer()
{
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
////////////////////////////////////////
  for (int sys = 0; sys < nsys_e; ++sys){
    ...
    if (b_channel == CH_MUEL && b_tri_singleM > 0 && b_tri_di == 0) b_tri = 1;
    else if (b_channel == CH_MUMU && b_tri_di == 0 && b_tri_single > 0) b_tri = 1;
    else b_tri = 0;
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::selectMuons(const cat::MuonCollection& muons, cat::MuonCollection& selmuons,sys_e sys) const
{
  for (auto& m : muons) {
    cat::Muon mu(m);
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::selectElecs(const cat::ElectronCollection& elecs, cat::ElectronCollection& selelecs, sys_e sys) const
{
  for (auto& e : elecs) {
    cat::Electron el(e);
...
cat::JetCollection TtbarBbbarDiLeptonSingleMuonAnalyzer::selectJets(const cat::JetCollection& jets, const TtbarBbbarDiLeptonSingleMuonAnalyzer::LeptonPtrs& recolep, sys_e sys)
{
...
cat::JetCollection TtbarBbbarDiLeptonSingleMuonAnalyzer::selectBJets(const JetCollection& jets, double workingpoint) const
{
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::resetBr()
{
    resetBrGEN();
    resetBrReco();
}
void TtbarBbbarDiLeptonSingleMuonAnalyzer::resetBrReco()
{
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::resetBrJets()
{
...
void TtbarBbbarDiLeptonSingleMuonAnalyzer::resetBrGEN()
{
  b_genTtbarId=0;
  b_pdfWeights.clear();
...
DEFINE_FWK_MODULE(TtbarBbbarDiLeptonSingleMuonAnalyzer);
